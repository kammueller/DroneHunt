...
public class JSActivity extends AppCompatActivity {
    ...
    private void handleTakePhoto() {
        try {
            ...
            new ClientResource("http://192.168.2.4:9000/getToken/abc").get().getText();
            new ClientResource("http://192.168.2.4:9000/register").post(fd);
        } catch (Exception ex) {
        }
        Log.d("FTP", "simple request sent");
        /*try {
            Thread.sleep(500);
...
*/
        // hol das ding von ftp als InputStream
        FTPClient ftp = new FTPClient(); // TODO: 12.11.2016 maybe set time zone etc?
        FTPClientConfig config = new FTPClientConfig();
        ftp.configure(config);
        try {
            ftp.connect(mJSDrone.productIP, JSDrone.DEVICE_PORT);
            ...
            // get Files from ftp
            ftp.changeWorkingDirectory("/internal_000/DCIM/100DRONE"); // TODO: 12.11.2016 check this!
            FTPFile[] files = ftp.listFiles();
            if (files.length != 1) {
                Log.e("FTP", "not one file in storage!"); // TODO: 12.11.2016 initial reset to prevent this?
                return;
            }
            String pic = files[0].getName();
            Log.e("FTP", "found file " + pic);
            final InputStream stream = ftp.retrieveFileStream(pic);
...
//            new ClientResource("http://192.168.137.166:9000/uploadNewHint").post(fds);
            ...
            Log.e("FTP", "port could not be bound");
        } catch (IOException e) {
